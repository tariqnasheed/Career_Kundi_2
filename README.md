...
CareerKundi is a full-stack, self-hostable web application that helps a job seeker go from "I found a posting" to "I applied with a tailored CV and prepped for the interview." The backend is a FastAPI service built around **LangGraph multi-agent pipelines** (Guardrail → Planner → Executor → Reflector, with feature-specific extra stages) that call a **local Ollama 8B** model for generation and **RAG/GraphRAG** for grounding. The frontend is a React 18 + TypeScript single-page app.

The default LLM provider is **local Ollama 8B** (`http://127.0.0.1:11434`). Set `LLM_PROVIDER=mock` for deterministic offline tests when Ollama is unavailable. Search still mocks when `SERPAPI_KEY` is blank. No Gemini API key is required for current local operation.
...
flowchart TB
  ...
  Backend -->|LLM_PROVIDER=ollama| Ollama[Local Ollama 8B]
...
## 11. AI / Local Ollama Usage

**Current LLM provider: Local Ollama 8B.**
Default local API: `http://127.0.0.1:11434`.
Default model: configurable 8B tag (`OLLAMA_MODEL_FLASH` / `OLLAMA_MODEL_PRO`, default `llama3.1:8b`).
No Gemini API required for current local operation.
No public cloud LLM dependency by default.
Use `LLM_PROVIDER=mock` for deterministic tests when Ollama is unavailable.

Local LLM output is not guaranteed correct and is never treated as verified truth.

- **Integration path**: `app/tools/llm.py` `OllamaProvider` calls Ollama `POST /api/generate` via httpx. Agents use `get_llm(tier)`.
- **Structured output**: JSON schema requests set Ollama `format` and instruct JSON-only replies; parse failures use a schema-minimal fallback (never pretend invalid JSON parsed).
- **Local setup**: install Ollama, run `ollama pull llama3.1:8b` (or set `OLLAMA_MODEL_*` to your installed 8B tag).
- **Offline/tests**: `LLM_PROVIDER=mock` → `DeterministicMockProvider`.
- **Error handling**: connection refused / missing model / timeout messages point to starting Ollama, `ollama pull <model>`, or `OLLAMA_REQUEST_TIMEOUT_SECONDS`.

Gemini mentions elsewhere in older docs are **legacy/deprecated** unless explicitly marked historical.
...
| `LLM_PROVIDER` | No (default `ollama`) | `ollama` / `mock` | Active LLM provider | No |
| `OLLAMA_BASE_URL` | No (default `http://127.0.0.1:11434`) | local Ollama API | No |
| `OLLAMA_MODEL_FLASH` | No (default `llama3.1:8b`) | Local 8B flash tier | No |
| `OLLAMA_MODEL_PRO` | No (default `llama3.1:8b`) | Local 8B pro tier | No |
| `OLLAMA_REQUEST_TIMEOUT_SECONDS` | No (default `120`) | Ollama HTTP timeout | No |
| `GEMINI_API_KEY` / `GEMINI_MODEL_*` | Deprecated legacy | ignored by provider selection | Yes (legacy only) |
